...
export default function BuyerDashboardPage() {
    ...
    setThreads((prev) =>
      prev.map((t) =>
        t.id === activeThread.id
          ? { ...t, messages: [...t.messages, optimisticMsg], lastMessage: body, lastMessageAt: optimisticMsg.createdAt }
          : t
      )
    );
    setActiveThread((t) => t ? { ...t, messages: [...t.messages, optimisticMsg] } : null);
  ...
  const tabs: { id: Tab; label: string; count?: number }[] = [
    ...
    { id: 'messages', label: 'Messages', count: threads.reduce((n, t) => n + t.unreadCount, 0) || undefined },
  ];
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            <div className="animate-pulse space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-white rounded-2xl border border-[#E3E0D9] h-48" />
                ))}
              </div>
            </div>
          </div>
      ...
      <>
        ...
        <main className="bg-[#FAFAF8] min-h-screen flex items-center justify-center">
          <div className="text-center">
            <p className="text-[14px] text-[#9E9A91]">{error || 'Please sign in to view your dashboard.'}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="bg-white rounded-2xl border border-[#E3E0D9] p-6 mb-6 flex
            ...
            <div className="flex-1 min-w-0">
              <h1 className="font-[family-name:var(--font-playfair)] text-[1.5rem]
              ...
              <div className="flex flex-wrap gap-3 mt-3">
                {[
                  ...
                  { value: orders.filter((o) => o.status === 'completed').length, label: 'Completed' },
                ].map(({ value, label }) => (
                  <div key={label} className="text-center">
                    <p className="font-[family-name:var(--font-playfair)] text-[1.25rem]
          ...
          {activeTab === 'orders' && (
            ...
            >
              {orders.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-[#C9C5BC]
                  p-12 text-center">
          ...
          {activeTab === 'watchlist' && (
            <div role="tabpanel" aria-label="Watchlist">
              {watchlist.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-[#C9C5BC]
                  p-12 text-center">
                  <p className="text-[14px] text-[#9E9A91]">Your watchlist is empty</p>
                  <Link href="/search" className="mt-3 inline-block">
                    <Button variant="secondary" size="sm">Find items to watch</Button>
                  </Link>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {activeTab === 'messages' && (
            ...
            >
              ...
              <div className="border-b md:border-b-0 md:border-r border-[#E3E0D9]">
                <div className="px-4 py-3 border-b border-[#E3E0D9] flex items-center
                  justify-between">
                  <h2 className="text-[13px] font-semibold text-[#141414]">Conversations</h2>
                </div>
                {threads.map((thread) => (
                  <button
                    key={thread.id}
                  ...
                  >
                    <Avatar name={thread.otherPartyName} size="sm" className="mt-0.5 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {thread.unreadCount > 0 && (
                          <span className="shrink-0 w-4 h-4 rounded-full bg-[#D4A843] text-white
                  ...
                  <div className="flex-1 overflow-y-auto p-5 space-y-4 max-h-[360px]">
                    {activeThread.messages.map((msg) => {
                      ...
                      return (
                        ...
                        >
                          {!isMe && (
                            <Avatar name={msg.senderName} size="sm" className="shrink-0 mt-0.5" />
                          )}
...
function WatchlistCard({
  ...
  return (
    <article className="bg-white rounded-2xl border border-[#E3E0D9] overflow-hidden
  ...
  return (
    <div className="bg-[#FFF9EC] border border-[#D4A843]/30 rounded-2xl p-4 mb-6 flex items-center justify-between gap-4">
      ...
      {status === 'sent' ? (
        ...
        >
          {status === 'loading' ? 'Sending…' : status === 'error' ? 'Try again' : 'Resend email'}
        </button>
      )}
    </div>
  );
