...
export default async function FinancePage() {
  ...
  const refundRate =
    totalOrdersMonth > 0
      ? ((refundsMonthCount / totalOrdersMonth) * 100).toFixed(1)
      : "0.0";
  ...
  const kpiRows = [
    [
      {
        label: "GMV Today",
        value: formatPrice((gmvToday._sum.totalNzd ?? 0) / 100),
      },
      {
        label: "GMV This Week",
        value: formatPrice((gmvWeek._sum.totalNzd ?? 0) / 100),
      },
      {
        label: "GMV This Month",
        value: formatPrice((gmvMonth._sum.totalNzd ?? 0) / 100),
      },
      {
        label: "GMV This Year",
        value: formatPrice((gmvYear._sum.totalNzd ?? 0) / 100),
      },
    ],
    [
      {
      ...
      },
      {
        ...
        value: formatPrice((pendingPayoutsAgg._sum.amountNzd ?? 0) / 100),
      },
    ],
    [
      {
        label: "Refunds This Month",
        value: refundsMonthCount.toLocaleString("en-NZ"),
      },
      {
        label: "Refund Value",
        value: formatPrice((refundsMonthAgg._sum.totalNzd ?? 0) / 100),
      },
      {
        label: "Refund Rate",
        value: `${refundRate}%`,
        alert: parseFloat(refundRate) > 5,
      },
      {
        label: "Failed Payouts",
        value: failedPayouts.toLocaleString("en-NZ"),
      ...
      <div className="bg-[#141414] text-white">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <p className="text-white/50 text-[13.5px]">
            Revenue, payouts and transaction overview
          </p>
        </div>
      </div>
              ...
              <table className="w-full text-[12.5px]">
                <thead>
                  <tr className="border-b border-[#F0EDE8] bg-[#FAFAF8]">
                    {["Rank", "Seller", "Verified", "Orders", "Revenue"].map(
                      (h) => (
                        <th
                          key={h}
                  ...
                  {topSellersGrouped.map((s, idx) => {
                    ...
                    return (
                      <tr key={s.sellerId} className="hover:bg-[#FAFAF8]">
                        <td className="px-4 py-3 font-semibold text-[#9E9A91]">
                          #{idx + 1}
                        </td>
                        ...
                        <td className="px-4 py-3 font-semibold text-[#141414]">
                          {formatPrice((s._sum.totalNzd ?? 0) / 100)}
                        </td>
                      </tr>
                    );
                  })}
                ...
                {transactions.map((t) => (
                  <tr key={t.id} className="hover:bg-[#FAFAF8]">
                    ...
                    <td className="px-4 py-3 text-[#73706A] whitespace-nowrap">
                      {t.completedAt
                        ? new Date(t.completedAt).toLocaleDateString("en-NZ", {
                            day: "numeric",
                            month: "short",
                          })
                        : "—"}
                    </td>
                    <td className="px-4 py-3 text-[#141414] max-w-[160px] truncate">
                      {t.listing.title}
                    </td>
                    ...
                    <td className="px-4 py-3 font-semibold text-[#141414]">
                      {formatPrice(t.totalNzd / 100)}
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                          t.payout?.status === "PAID"
                            ? "bg-emerald-50 text-emerald-700"
                            : t.payout?.status === "PROCESSING"
                ...
                <tbody className="divide-y divide-[#F8F7F4]">
                  {pendingPayouts.map((p) => (
                    <tr key={p.id} className="hover:bg-[#FAFAF8]">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-semibold text-[#141414]">
                          {p.order.seller.displayName}
                        </p>
                        <p className="text-[11px] text-[#9E9A91]">
                          {p.order.seller.email}
                        </p>
                      </td>
                      <td className="px-4 py-3 font-semibold text-[#141414]">
                        {formatPrice(p.amountNzd / 100)}
                      </td>
                      <td className="px-4 py-3 text-[#73706A] max-w-[140px] truncate">
                        {p.order.listing.title}
                      </td>
                      <td className="px-4 py-3 text-[#9E9A91] whitespace-nowrap">
                        {p.initiatedAt
                          ? new Date(p.initiatedAt).toLocaleDateString("en-NZ")
                          : "—"}
                      </td>
