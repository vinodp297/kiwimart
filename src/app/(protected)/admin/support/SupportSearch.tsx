    ...
    <div className="space-y-6">
      ...
      {notFound && (
        <div className="bg-white rounded-2xl border border-[#E3E0D9] p-8 text-center">
          <p className="text-[#9E9A91] text-[13.5px]">
      ...
      {userResult && (
        <div className="bg-white rounded-2xl border border-[#E3E0D9] p-6">
            ...
            {[
              {
                ...
                value: userResult.emailVerified ? "✓ Yes" : "✗ No",
              },
              {
                label: "Phone Verified",
                value: userResult.isPhoneVerified ? "✓ Yes" : "✗ No",
              },
              {
                label: "ID Verified",
                value: userResult.idVerified ? "✓ Yes" : "✗ No",
              },
              {
                label: "Seller",
                value: userResult.isSellerEnabled ? "Enabled" : "Disabled",
              },
              {
                label: "Stripe",
                value: userResult.isStripeOnboarded
                  ? "Connected"
                  : "Not set up",
              },
              ...
              {
                label: "Listings",
                value: userResult._count.listings.toString(),
              },
              {
                label: "Purchases",
                value: userResult._count.buyerOrders.toString(),
              },
              {
                label: "Sales",
                value: userResult._count.sellerOrders.toString(),
              },
            ].map(({ label, value }) => (
              <div key={label} className="bg-[#F8F7F4] rounded-xl p-3">
                <p className="text-[10px] font-semibold text-[#9E9A91] uppercase tracking-wider mb-0.5">
                  {label}
                </p>
          ...
          <p className="text-[11px] text-[#C9C5BC]">
            Member since{" "}
            {new Date(userResult.createdAt).toLocaleDateString("en-NZ", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })}
        ...
        <div className="bg-white rounded-2xl border border-[#E3E0D9] p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-[12px] font-mono text-[#9E9A91]">
                Order #{orderResult.id}
              </p>
            </div>
            <span
              className={`px-2.5 py-1 rounded-full text-[11px] font-bold ${ORDER_STATUS_COLORS[orderResult.status] ?? "bg-[#F8F7F4] text-[#9E9A91]"}`}
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
            {[
              {
                label: "Buyer",
                value: `${orderResult.buyer.displayName} (${orderResult.buyer.email})`,
              },
              {
                label: "Seller",
                value: `${orderResult.seller.displayName} (${orderResult.seller.email})`,
              },
              {
                label: "Total",
                value: formatPrice(orderResult.totalNzd / 100),
              },
              { label: "Item", value: formatPrice(orderResult.itemNzd / 100) },
              {
                label: "Shipping",
                value:
                  orderResult.shippingNzd > 0
                    ? formatPrice(orderResult.shippingNzd / 100)
                    : "Free",
              },
              {
                label: "Created",
                value: new Date(orderResult.createdAt).toLocaleDateString(
                  "en-NZ",
                ),
              },
            ].map(({ label, value }) => (
          ...
          {orderResult.trackingNumber && (
            <p className="text-[12px] text-[#73706A]">
              Tracking:{" "}
              <span className="font-mono">{orderResult.trackingNumber}</span>
            </p>
          )}
