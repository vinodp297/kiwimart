...
export default function TeamActions({ members, currentUserId }: Props) {
  ...
  async function handleChangeRole(memberId: string, memberName: string, currentRole: AdminRole, newRole: AdminRole) {
    if (newRole === currentRole) return;
    if (!confirm(`Change ${memberName} from ${getRoleDisplayName(currentRole)} to ${getRoleDisplayName(newRole)}?`)) return;
    setActionLoading(memberId + '_role');
    setActionError('');
    const result = await changeAdminRole(memberId, newRole);
    if (!result.success) setActionError(result.error ?? 'Failed to change role');
    else router.refresh();
    setActionLoading(null);
  }

  async function handleRevoke(memberId: string, memberName: string) {
    if (!confirm(`Revoke admin access for ${memberName}? They will lose all admin permissions immediately.`)) return;
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] text-[#9E9A91] mt-0.5">{members.length} admin{members.length === 1 ? '' : 's'}</p>
        </div>
        <button
              ...
              {members.map((m) => (
                <tr key={m.id} className="hover:bg-[#FAFAF8] transition-colors">
                  <td className="px-5 py-3 font-semibold text-[#141414]">
                    {m.displayName}
                    {m.id === currentUserId && (
                      <span className="ml-2 text-[10px] text-[#9E9A91]">(you)</span>
                    )}
                  </td>
