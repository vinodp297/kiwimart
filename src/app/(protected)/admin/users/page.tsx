    ...
    <div className="bg-[#FAFAF8] min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E3E0D9] overflow-hidden">
            {loading ? (
              <div className="p-12 text-center text-[#9E9A91] text-[13px]">Loading users…</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-[12.5px]">
                  <thead>
                    <tr className="border-b border-[#F0EDE8] bg-[#FAFAF8]">
                      {['Username', 'Email', 'Region', 'Seller', 'Verified', 'Banned', 'Listings', 'Orders', 'Joined', 'Actions'].map((h) => (
                        <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-[#9E9A91] uppercase tracking-wide whitespace-nowrap">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-[#F8F7F4]">
                    {users.map((u) => (
                      <tr key={u.id} className={`hover:bg-[#FAFAF8] transition-colors ${u.isBanned ? 'opacity-60' : ''}`}>
                        <td className="px-4 py-3 font-medium text-[#141414]">@{u.username}</td>
                        <td className="px-4 py-3 text-[#73706A]">{u.email}</td>
                        <td className="px-4 py-3 text-[#73706A]">{u.region ?? '—'}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                            u.sellerEnabled ? 'bg-emerald-50 text-emerald-700' : 'bg-[#F8F7F4] text-[#9E9A91]'
                          }`}>
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                            u.idVerified ? 'bg-[#F5ECD4] text-[#8B6914]' : 'bg-[#F8F7F4] text-[#9E9A91]'
                          }`}>
                            {u.idVerified ? '✓' : '—'}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                            u.isBanned ? 'bg-red-50 text-red-700' : 'bg-[#F8F7F4] text-[#9E9A91]'
                          }`}>
                            {u.isBanned ? 'Yes' : 'No'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-[#73706A]">{u._count.listings}</td>
                        <td className="px-4 py-3 text-[#73706A]">{u._count.buyerOrders}</td>
                          ...
                          <div className="flex gap-1.5">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg text-[11px] font-semibold
                                ...
                                disabled:opacity-50"
                            >
                              {actionLoading === u.id + '_seller' ? '…' : u.sellerEnabled ? 'Disable seller' : 'Enable seller'}
                            </button>
                          </div>
                        </td>
                      </tr>
                    ...
                    {users.length === 0 && (
                      <tr>
                        <td colSpan={10} className="px-4 py-12 text-center text-[#9E9A91]">No users found</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <div className="flex justify-center gap-3 mt-4">
            ...
            <span className="flex items-center text-[12.5px] text-[#73706A]">Page {page}</span>
            <button
