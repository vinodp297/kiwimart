...
export default async function ModerationPage() {
  ...
  const { reports, resolvedToday, bannedUsers } =
    await adminService.getModerationData();
              ...
              <table className="w-full text-[12.5px]">
                <thead>
                  <tr className="border-b border-[#F0EDE8] bg-[#FAFAF8]">
                    {["User", "Email", "Banned", "Reason", "Actions"].map(
                      (h) => (
                        <th
                          key={h}
                          className="px-4 py-3 text-left text-[11px] font-semibold text-[#9E9A91] uppercase tracking-wide"
                        >
                          {h}
                        </th>
                      ),
                  ...
                  {bannedUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-[#FAFAF8]">
                      ...
                      <td className="px-4 py-3 text-[#9E9A91] whitespace-nowrap">
                        {u.bannedAt
                          ? new Date(u.bannedAt).toLocaleDateString("en-NZ")
                          : "—"}
                      </td>
                      <td className="px-4 py-3 text-[#73706A] max-w-[200px] truncate">
                        {u.bannedReason ?? "—"}
                      </td>
