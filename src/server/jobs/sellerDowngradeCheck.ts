...
export async function runSellerDowngradeCheck(): Promise<{
  ...
  const LOCK_KEY = "cron:seller-downgrade-check";
  const LOCK_TTL_SECONDS = 300;

  const lock = await acquireLock(LOCK_KEY, LOCK_TTL_SECONDS);
  if (!lock) {
    logger.info("seller_downgrade_check.skipped_lock_held", {
      reason:
  ...
  try {
    return await runWithRequestContext(
      { correlationId: `cron:runSellerDowngradeCheck:${Date.now()}` },
      async () => {
        const [disputeRateThresholdPct, openDisputeThreshold] =
          await Promise.all([
        ...
        for (const s of sellersAtRisk) {
          ...
          const completionRate =
            s.trustMetrics.totalOrders > 0
              ? (s.trustMetrics.completedOrders / s.trustMetrics.totalOrders) *
                100
              : 0;
        ...
        for (const s of sellersWithOpenDisputes) {
          ...
          if (existing) {
            ...
            const completionRate =
              s.trustMetrics.totalOrders > 0
                ? (s.trustMetrics.completedOrders /
                    s.trustMetrics.totalOrders) *
                  100
                : 0;
            candidateMap.set(s.id, {
              id: s.id,
