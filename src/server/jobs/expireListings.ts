...
// paymentDeadline is set 24 hours out. If the buyer hasn't paid by then,
...
export async function releaseExpiredOfferReservations(): Promise<{
  ...
  if (expiredOffers.length === 0) {
    logger.info('job.release_expired_offer_reservations.completed', { released: 0 })
    return { released: 0, errors: 0 }
  }

  // Exclude listings that already have a paid order (buyer completed checkout)
  const paidListingIds = await db.order.findMany({
    where: {
      listingId: { in: expiredOffers.map((o) => o.listingId) },
      status: { in: ['PAYMENT_HELD', 'DISPATCHED', 'DELIVERED', 'COMPLETED', 'DISPUTED'] },
    },
  ...
  logger.info('job.release_expired_offer_reservations.completed', { released, errors })
