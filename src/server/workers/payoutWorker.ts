  ...
  const worker = new Worker<PayoutJobData>(
    ...
    async (job) => {
      ...
      return runWithRequestContext({ correlationId }, async () => {
        ...
        // same order simultaneously. TTL is 120 s; heartbeat renews every 40 s
        // so long Stripe API calls (which can take 30–60 s) cannot cause the
        // lock to expire and allow a concurrent worker to start a duplicate transfer.
        // withLockAndHeartbeat throws AppError(LOCK_CONTENTION) on miss —
        // BullMQ catches the error and marks the job FAILED for retry.
        const lockKey = `payout:${orderId}`;
        return await withLockAndHeartbeat(
          lockKey,
          async () => {
            // ── Idempotency check ───────────────────────────────────────────
            ...
            const sellerUser =
              await userRepository.findSellerForPayout(sellerId);
