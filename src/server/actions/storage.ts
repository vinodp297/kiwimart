...
export async function getStorageStats(): Promise<ActionResult<StorageStats>> {
  ...
  const compressionSavingsBytes = totalOriginalSizeBytes > 0
    ? totalOriginalSizeBytes - totalSizeBytes
    : 0;
  const compressionRatio = totalOriginalSizeBytes > 0
    ? Number(((1 - totalSizeBytes / totalOriginalSizeBytes) * 100).toFixed(1))
    : 0;
