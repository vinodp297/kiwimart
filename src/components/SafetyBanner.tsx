  ...
  if (variant === "compact") {
    return (
      ...
      >
        ...
        Protected by {process.env.NEXT_PUBLIC_APP_NAME ?? "Buyzi"} Buyer
        Guarantee.{" "}
        <Link href="/trust" className="underline hover:text-[#141414]">
          Learn more
        </Link>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <p className="text-[12px] text-[#8B6914] leading-relaxed">
        <strong>Stay safe:</strong> Always use{" "}
        {process.env.NEXT_PUBLIC_APP_NAME ?? "Buyzi"}&apos;s secure payment to
        keep your purchase covered by{" "}
        {process.env.NEXT_PUBLIC_BUYER_PROTECTION_DISPLAY ?? "$3,000"} buyer
        protection. Never pay by bank transfer outside the platform.{" "}
        <Link
          href="/safety"
          className="underline hover:text-[#141414] transition-colors"
        >
          Safety guide
        </Link>
        {" · "}
