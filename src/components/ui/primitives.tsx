...
const BUTTON_VARIANTS: Record<ButtonVariant, string> = {
  ...
  gold:
    'bg-[#D4A843] text-[#141414] hover:bg-[#B8912E] hover:text-white focus-visible:ring-[#D4A843] shadow-sm shadow-[#D4A843]/30',
};
  ...
  ({ label, error, hint, leftAddon, rightAddon, className = '', id, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        {label && (
          <label htmlFor={inputId} className="text-[12.5px] font-semibold text-[#141414]">
            {label}
        ...
        <div className="relative flex items-center">
          ...
          />
          {rightAddon && (
            <div className="absolute right-3 text-[#9E9A91]">
              {rightAddon}
            </div>
          )}
        </div>
        {error && <p className="text-[11.5px] text-red-500 font-medium">{error}</p>}
        {hint && !error && <p className="text-[11.5px] text-[#9E9A91]">{hint}</p>}
      </div>
    );
  }
);
      ...
      <div className="flex flex-col gap-1.5">
        {label && (
          <div className="flex items-center justify-between">
            ...
            {charCount && (
              <span className={`text-[11px] ${charCount.current > charCount.max * 0.9 ? 'text-amber-600' : 'text-[#9E9A91]'}`}>
...
export function Avatar({
  ...
  const sizes = { sm: 'w-7 h-7 text-[11px]', md: 'w-10 h-10 text-[14px]', lg: 'w-14 h-14 text-[18px]', xl: 'w-20 h-20 text-[24px]' };
  const initials = name
    .split(' ')
    .map((w) => w[0])
    .join('')
          ...
          <li key={i} className="flex items-center gap-1.5">
            {i > 0 && (
              <svg aria-hidden width="10" height="10" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2.5" className="text-[#C9C5BC]">
    ...
    <div className="flex items-center gap-1.5">
      <div className="flex items-center gap-0.5" aria-label={`${rating} out of 5 stars`}>
        {Array.from({ length: full }).map((_, i) => (
          <svg key={`f${i}`} width={starSize} height={starSize} viewBox="0 0 24 24" fill="#D4A843">
        ...
        {Array.from({ length: empty }).map((_, i) => (
          <svg key={`e${i}`} width={starSize} height={starSize} viewBox="0 0 24 24" fill="#E3E0D9">
            <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
          </svg>
        ))}
      </div>
      <span className={`font-semibold text-[#141414] ${size === 'sm' ? 'text-[12.5px]' : 'text-[14px]'}`}>
        {rating.toFixed(1)}
      </span>
      {showCount && reviewCount !== undefined && (
        <span className={`text-[#9E9A91] ${size === 'sm' ? 'text-[11.5px]' : 'text-[13px]'}`}>
...
const ALERT_ICONS: Record<AlertVariant, React.ReactNode> = {
  ...
  error: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>,
};
    ...
    <div className="mt-1.5 space-y-1.5">
      ...
      {level.label && (
        <p className={`text-[11px] font-semibold ${
          score === 1 ? 'text-red-600' : score === 2 ? 'text-amber-600' : score === 3 ? 'text-sky-600' : 'text-emerald-600'
        }`}>
