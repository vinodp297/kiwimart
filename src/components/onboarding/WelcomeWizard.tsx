...
const INTENTS: { value: Intent; emoji: string; title: string; subtitle: string }[] = [
  { value: 'BUY',  emoji: '🛍',  title: 'Buy items',       subtitle: 'I want to find great deals from NZ sellers' },
  { value: 'SELL', emoji: '📦',  title: 'Sell items',      subtitle: 'I want to list items and earn money' },
  { value: 'BOTH', emoji: '🔄',  title: 'Buy & sell',      subtitle: 'I want to do both — buy and sell' },
];

export default function WelcomeWizard({ displayName }: WelcomeWizardProps) {
  ...
  function handleIntentNext() {
    if (!intent) { setError('Please choose an option to continue.'); return; }
    setError('');
    setStep(2);
  }
  ...
  function handleRegionNext() {
    ...
    startTransition(async () => {
      const result = await completeOnboarding({ intent: intent!, region: region || undefined });
      if (result.success) {
        setStep(3);
      } else {
    ...
    <main className="min-h-screen bg-[#FAFAF8] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-[520px]">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          <div className="w-8 h-8 rounded-full bg-[#141414] flex items-center justify-center text-[#D4A843] text-sm font-bold">K</div>
        ...
        <div className="bg-white rounded-2xl border border-[#E3E0D9] shadow-sm p-8">
          ...
          {step === 1 && (
            <>
              ...
              <p className="text-[13.5px] text-[#73706A] mb-6">
                Let&apos;s personalise your KiwiMart experience. What brings you here?
              </p>
              ...
              <div className="space-y-3 mb-6">
                {INTENTS.map((opt) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-semibold text-[14px] text-[#141414]">{opt.title}</p>
                      <p className="text-[12.5px] text-[#73706A] mt-0.5">{opt.subtitle}</p>
                    </div>
                    {intent === opt.value && (
                      <svg className="ml-auto text-[#D4A843] shrink-0" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
          ...
          {step === 2 && (
            <>
              ...
              <p className="text-[13.5px] text-[#73706A] mb-6">
                We&apos;ll show you listings and sellers near you. You can change this later.
              </p>
              ...
              <div className="mb-6">
                <label className="block text-[12.5px] font-semibold text-[#141414] mb-1.5">
                  Region <span className="text-[#9E9A91] font-normal">(optional)</span>
                </label>
                <select
                  value={region}
          ...
          {step === 3 && (
            <div className="text-center">
              ...
              <p className="text-[13.5px] text-[#73706A] mb-6 leading-relaxed">
                Your KiwiMart account is ready. Start exploring thousands of listings from
                NZ sellers or list your first item in minutes.
              </p>
              ...
              <div className="bg-[#FAFAF8] border border-[#E3E0D9] rounded-xl p-4 text-left text-[13px] text-[#73706A] space-y-2 mb-6">
                {(intent === 'SELL' || intent === 'BOTH') && (
                  <p>📦 <strong className="text-[#141414]">List an item</strong> — sell in under 2 minutes, $0 listing fee</p>
                )}
                {(intent === 'BUY' || intent === 'BOTH') && (
                  <p>🔍 <strong className="text-[#141414]">Browse listings</strong> — find great deals near you</p>
                )}
                <p>🛡 <strong className="text-[#141414]">Buyer protection</strong> — every purchase backed up to $3,000</p>
              </div>
