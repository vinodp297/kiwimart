 ...
 */
export async function updateSellerResponseMetrics(sellerId: string): Promise<void> {
  try {
    ...
    for (const thread of threads) {
      ...
      const sellerReply = msgs.slice(buyerFirstIdx + 1).find(
        (m) => m.senderId === sellerId
      )
      if (!sellerReply) continue

      const delta = sellerReply.createdAt.getTime() - msgs[buyerFirstIdx].createdAt.getTime()
      if (delta > 0) {
        replyMs.push(delta)
    ...
    const responseRate = totalBuyerThreads > 0
      ? (repliedWithin24h / totalBuyerThreads) * 100
      : 0
