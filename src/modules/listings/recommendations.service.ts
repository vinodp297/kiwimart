...
function mapListingRow(row: {
  id: string; title: string; priceNzd: number; condition: string;
  categoryId: string; subcategoryName: string | null;
  region: string; suburb: string;
  shippingOption: string; shippingNzd: number | null; offersEnabled: boolean;
  status: string; viewCount: number; watcherCount: number; createdAt: Date;
  images: { r2Key: string }[];
...
export const getMoreFromSeller = unstable_cache(
  async (sellerId: string, excludeListingId: string): Promise<ListingCard[]> => {
...
export const getSimilarListings = unstable_cache(
  async (listingId: string, categoryId: string, priceNzd: number, sellerId: string): Promise<ListingCard[]> => {
