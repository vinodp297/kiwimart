...
describe("Stripe seller onboarding sync", () => {
  ...
  it("sets isStripeOnboarded=true when ALL 3 conditions are true", () => {
  ...
  it("sets isStripeOnboarded=false when payouts_enabled is false", () => {
    const status = computeOnboardingStatus({
      details_submitted: true,
      charges_enabled: true,
  ...
  it("sets isStripeOnboarded=false when charges_enabled is false", () => {
    const status = computeOnboardingStatus({
      details_submitted: true,
  ...
  it("sets isStripeOnboarded=false when details_submitted is false", () => {
    const status = computeOnboardingStatus({
  ...
  it("syncs FALSE when account regresses (bidirectional)", async () => {
    ...
    await db.user.updateMany({
      ...
      data: {
        ...
        isStripeChargesEnabled: chargesEnabled,
        isStripePayoutsEnabled: payoutsEnabled,
      },
    });
